...
bool OSHMonitor::exportCSVWithTWA(const String &filename) {
    ...
    // Check if export should be disabled due to excessive duration
    if (_lastTWAExport.exceedsMaxDuration) {
        Serial.println("[ERROR] Dataset exceeds 10 hours - TWA export disabled");
        Serial.printf("[ERROR] Data coverage: %.1f hours\n", _lastTWAExport.dataCoverageHours);
        Serial.println("[ERROR] Please download data and analyze on desktop");
        return false;
    }
...
bool OSHMonitor::writeExportHeader(File &file) {
    ...
    // Determine appropriate report title based on data duration
    if (_lastTWAExport.dataCoverageHours > 8.0) {
        file.printf("# %.1f-Hour Time-Weighted Average Report\n", _lastTWAExport.dataCoverageHours);
    } else {
    ...
    // Adjust compliance message based on data duration
    if (_lastTWAExport.dataCoverageHours >= MIN_OSHA_HOURS && _lastTWAExport.dataCoverageHours <= 10.0) {
        file.printf("# OSHA Compliant: YES (%.1f hours)\n", _lastTWAExport.dataCoverageHours);
    } else if (_lastTWAExport.dataCoverageHours > 10.0) {
        file.printf("# OSHA Compliant: WARNING - Exceeds single shift duration (%.1f hours)\n", _lastTWAExport.dataCoverageHours);
        file.println("# Note: Dataset spans multiple shifts - use desktop analyzer for per-shift TWA");
    } else {
        file.printf("# OSHA Compliant: NO (< 8 hours - insufficient data: %.1f hours)\n", _lastTWAExport.dataCoverageHours);
    }
    ...
    // Format TWA labels based on duration
    const char* twaLabel = (_lastTWAExport.dataCoverageHours > 8.0) ? "%.1f-hr TWA" : "8-hr TWA";
    
    if (it_pm1 != _lastTWAExport.parameterTWAs.end()) {
        file.printf("# PM1.0 ");
        file.printf(twaLabel, _lastTWAExport.dataCoverageHours);
        file.printf(": %.3f µg/m³\n", it_pm1->second);
    }
    ...
    if (_lastTWAExport.dataCoverageHours > 10.0) {
        file.println("#");
        file.println("# WARNING: Dataset exceeds single shift duration");
        file.println("# Recommendation: Use desktop analyzer for per-shift TWA calculations");
    }
